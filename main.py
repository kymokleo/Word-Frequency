...
def write_json_output(frequencies, output_file=None):
    ...
    sorted_frequencies = dict(sorted(
        frequencies.items(),
        key=lambda x: (-x[1], x[0])  # Negative frequency for descending order, then word alphabetically
    ))
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Count word frequencies and output as JSON')
